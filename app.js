...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const supported = ['es','en','pt','fr','ru','ja','zh'];
  const auto = (urlLang && supported.includes(urlLang) && urlLang)
    || saved
...
/* ── Traducción de etiquetas dinámicas de los directorios ──
   DIR_MAP_LABEL y DIR_TAGS ahora viven en langs.js (fuente única, compartida
   también por las subpáginas). Aquí solo se referencian vía window. ── */
const DIR_MAP_LABEL = window.DIR_MAP_LABEL || {};
const DIR_TAGS = window.DIR_TAGS || {};
function dirT(text) {
  if (!text) return text;
...
(function () {
  ...
  function init() {
    ...
    // Antes aparecía a los 1.6s, compitiendo con la primera impresión del
    // hero y el aviso de temporada. Se retrasa a 4s para no saturar la
    // pantalla en los primeros segundos de la visita.
