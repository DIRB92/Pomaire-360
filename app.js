...
/* ─── Profile Modal (usado por directory-loader.js) ─── */
const PROFILES = {};
...
const san = (u) => { if(!u)return''; u=String(u).trim(); return /^(https?:\/\/|tel:|mailto:)/i.test(u)?u:/^\/[^\/]/.test(u)?u:''; };
const name = esc(it.n); const addr = esc(it.a); const desc = esc(it.desc);
const hours = esc(it.hours); const phone = esc(it.p);
const ig = esc((it.ig||'').replace(/^@/,'')); const web = san(it.web);
const fb = san(it.fb); const wsp = (it.wsp||'').replace(/[^0-9]/g,'');
const map = san(it.map); const mapsUrl = map || 'https://maps.google.com/?q='+encodeURIComponent(it.a+', Pomaire, Chile');
let links = `<a class="pf-link" href="${mapsUrl}" target="_blank" rel="noopener">🗺️ Mapa</a>`;
if(phone) links += `<a class="pf-link" href="tel:${it.p}">📞 ${phone}</a>`;
if(wsp) links += `<a class="pf-link" href="https://wa.me/${wsp}" target="_blank" rel="noopener">💬 WhatsApp</a>`;
if(ig) links += `<a class="pf-link" href="https://instagram.com/${encodeURIComponent(ig)}" target="_blank" rel="noopener">📷 Instagram</a>`;
if(web) links += `<a class="pf-link" href="${web}" target="_blank" rel="noopener">🌐 Web</a>`;
if(fb) links += `<a class="pf-link" href="${fb}" target="_blank" rel="noopener">📘 Facebook</a>`;
let gallery = '';
if(it.photos && it.photos.length) gallery = '<div class="pf-gallery">' + it.photos.map(u=>`<img src="${san(u)}" alt="${name}" loading="lazy">`).join('') + '</div>';
body.innerHTML = `${gallery}<div class="pf-head"><h3>${name}</h3></div>${desc?`<p class="pf-desc">${desc}</p>`:''}<div class="pf-meta"><div>📍 ${addr}</div>${hours?`<div>🕒 ${hours}</div>`:''}</div><div class="pf-links">${links}</div>`;
modal.classList.add('open'); document.body.style.overflow = 'hidden';
}
function closeProfile() { const m=document.getElementById('profileModal'); if(m)m.classList.remove('open'); document.body.style.overflow=''; }
function selectPlan(plan) { if(plan==='gratis') window.open('https://app.pomaire360.cl/auth/login','_blank','noopener'); else window.open('https://app.pomaire360.cl/planes','_blank','noopener'); }
